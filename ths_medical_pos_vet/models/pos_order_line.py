...
class PosOrderLine(models.Model):
	...
	def _check_vet_pet_owner_consistency(self):
		...
		for line in self:
			if line.order_id.ths_pet_owner_id:
				# Check single pet assignment
				if line.patient_ids:
					if line.patient_ids.ths_pet_owner_id != line.order_id.ths_pet_owner_id:
						raise ValidationError(
							_("Pet '%s' does not belong to pet owner '%s'") %
							(line.patient_ids.name, line.order_id.ths_pet_owner_id.name)
						)

				# Check multiple pets assignment
				if line.patient_ids:
					for pet in line.patient_ids:
						if pet.ths_pet_owner_id != line.order_id.ths_pet_owner_id:
							raise ValidationError(
								_("Pet '%s' does not belong to pet owner '%s'") %
								(pet.name, line.order_id.ths_pet_owner_id.name)
							)
	...
	def _onchange_vet_patients_sync(self):
		...
		if self.patient_ids:
			...
			self.ths_patient_id = self.patient_ids[0]
		else:
	...
	def _get_patient_info_for_reporting(self):
		...
		for pet in self.patient_ids:
			...
			if pet.ths_species_id:
				pet_info.append(f"Species: {pet.ths_species_id.name}")

			# Add pet owner information
			if pet.ths_pet_owner_id:
				pet_info.append(f"Owner: {pet.ths_pet_owner_id.name}")
