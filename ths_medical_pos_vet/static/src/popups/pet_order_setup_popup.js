...
export class PetOrderSetupPopup extends Component {
    ...
    setup() {
        ...
        if (this.isNewOrder) {
            ...
            this.availablePractitioners = this.pos.models["appointment.resource"]?.getAll().filter(r =>
                r.ths_resource_category === 'practitioner' && r.active
            ) || [];
            this.availableRooms = this.pos.models["appointment.resource"]?.getAll().filter(r =>
                r.ths_resource_category === 'location' && r.active
            ) || [];
        }
    ...
    getOwnerPets() {
        ...
        // Get all pets for this owner from preloaded data
        const allPartners = this.pos.models["res.partner"]?.getAll() || [];
        return allPartners.filter(partner =>
            partner.ths_pet_owner_id &&
            partner.ths_pet_owner_id[0] === this.partner.id &&
            partner.active &&
    ...
    // Helper methods
    getSpeciesName(pet) {
        if (pet.ths_species_id && Array.isArray(pet.ths_species_id)) {
            return pet.ths_species_id[1];
        }
        return '';
    }
    ...
    getMembershipStatus(pet) {
        // Check if pet has active membership using preloaded data
        const memberships = this.pos.models["vet.pet.membership"]?.getAll() || [];
        const activeMembership = memberships.find(m =>
            m.patient_ids &&
            m.patient_ids.some(p => {
                const petId = Array.isArray(p) ? p[0] : (p.id || p);
                return petId === pet.id;
            }) &&
            m.state === 'running' &&
            m.is_paid
        ...
        return activeMembership ? 'active' : 'none';
    }

    getSpeciesColor(pet) {
        if (pet.ths_species_id && Array.isArray(pet.ths_species_id)) {
            const speciesId = pet.ths_species_id[0];
            const species = this.pos.models["ths.species"]?.get(speciesId);
            return species ? species.color || 0 : 0;
        }
        return 0;
    }
