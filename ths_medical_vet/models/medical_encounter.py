...
class ThsMedicalBaseEncounter(models.Model):
	...
	def _find_or_create_daily_encounter(
		...
		# Update ths_pet_owner_id on the encounter if it's provided and different
		if pet_owner_id and (not encounter.ths_pet_owner_id or encounter.ths_pet_owner_id.id != pet_owner_id):
			encounter.write({'ths_pet_owner_id': pet_owner_id})
	...
	@api.depends('patient_ids', 'patient_ids.name', 'patient_ids.ths_species_id', 'patient_ids.ths_species_id.name')
	def _compute_pets_summary(self):
		for encounter in self:
			...
			if not pets:
				...
				species_name = pets[0].ths_species_id.name if pets[0].ths_species_id and hasattr(pets[0].ths_species_id,
																								 'name') else ''
				...
				species = [s.name for s in pets.mapped('ths_species_id') if s and hasattr(s, 'name')]
