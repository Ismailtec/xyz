...
class ThsPendingPosItem(models.Model):
	...
	# Computed domain string for pets based on selected pets on the encounter
	...
	@api.depends('ths_pet_owner_id', 'encounter_id.patient_ids')
	def _compute_available_patient_domain(self):
		"""Compute domain for pets based on selected owner and/or pets already on the encounter header.
        - If encounter has patient_ids, filter strictly by those.
        - If encounter has no patient_ids (or no encounter), filter by owner."""
		for rec in self:
			domain = [('ths_partner_type_id.name', '=', 'Pet')]  # Base filter: ensure it's a 'Pet'

			# Flag to check if encounter-specific pet filtering was applied
			filtered_by_encounter_pets = False

			# First, check if the encounter has specific pets linked. This is the primary restriction.
			if rec.encounter_id and rec.encounter_id.patient_ids:
				...
				if encounter_pet_ids:
					# ONLY add this constraint if there are actual patient_ids on the encounter header
					domain.append(('id', 'in', encounter_pet_ids))
					filtered_by_encounter_pets = True
			# If encounter_id exists but patient_ids is empty, we do NOT add the ('id', 'in', [])
			# constraint here. Instead, it falls through to the owner filter below.

			# Second, if we haven't already filtered by specific encounter pets,
			# apply the owner filter if an owner is selected on the line.
			# This ensures that if the encounter has no patients, the owner filter still works.
			if not filtered_by_encounter_pets and rec.ths_pet_owner_id:
				domain.append(('ths_pet_owner_id', '=', rec.ths_pet_owner_id.id))
		...
		for vals in vals_list:
			if 'patient_ids' in vals and not vals.get('partner_id'):
				...
				if first_patient_id:
					...
					if patient_record.exists() and patient_record.ths_pet_owner_id:
						vals['partner_id'] = patient_record.ths_pet_owner_id.id
