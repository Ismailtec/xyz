...
class ParkCheckin(models.Model):
    ...
    membership_valid = fields.Boolean(
        ...
        store=True,
        help="Whether the membership allows park access"
    )
    ...
    @api.depends('partner_id.membership_state')
    def _compute_membership_valid(self):
        """Check if membership allows park access"""
        for record in self:
            record.membership_valid = record.partner_id.membership_state in ['paid', 'free']
    ...
    def _check_membership_access(self):
        ...
        for record in self:
            if not record.membership_valid:
                raise ValidationError(_(
                    "Pet owner '%s' does not have valid membership for park access. "
                    "Current membership status: %s",
                    record.partner_id.name,
                    record.partner_id.membership_state
                ))
