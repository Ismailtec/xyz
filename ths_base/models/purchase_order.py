# In ths_base/models/purchase_order.py
# -*- coding: utf-8 -*-
...
class PurchaseOrder(models.Model):
    ...
    ths_effective_date = fields.Date(
        string="Date",  # Label it simply as "Date" as requested
        copy=False,
        index=True,
    ...
    ths_stock_landed_cost_ids = fields.One2many(
        'stock.landed.cost',
        'purchase_order_id',  # This field needs to be added to stock.landed.cost
        string='Landed Costs',
        copy=False,
        readonly=True,
    ...
    # def _get_hide_taxes_setting_bool(self):
    #     """ Helper to get boolean value from config """
    #     hide_taxes_param = self.env['ir.config_parameter'].sudo().get_param('ths_base.ths_hide_taxes', 'False')
    #     return hide_taxes_param.lower() == 'true'

    # def _compute_hide_taxes(self):
    #     """ Compute the string value ('show' or 'hide') for the attributes """
    #     hide_taxes_bool = self._get_hide_taxes_setting_bool()
    #     # If setting is True (hide default), return 'show' (for optional)
    #     # If setting is False (show default), return 'hide' (for optional)
    #     optional_value = "show" if hide_taxes_bool else "hide"
    #     for order in self:
    #         order.ths_hide_taxes = optional_value
    ...
    #         if not self.partner_id.ths_partner_type_id or \
    #                 self.partner_id.ths_partner_type_id != vendor_type:
    #             self.partner_id.ths_partner_type_id = vendor_type
    ...
    def _create_or_update_landed_cost_from_po(self):
        ...
        if not po_lc_lines:
            ...
            # If no LC lines on PO, should we delete existing draft LCs linked?
            # Potentially dangerous if user manually created one. Let's skip deletion for now.
            # existing_draft_lcs = self.ths_stock_landed_cost_ids.filtered(lambda lc: lc.state == 'draft')
