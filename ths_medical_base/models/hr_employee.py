...
class HrEmployee(models.Model):
    ...
    def _compute_ths_is_medical(self):
        ...
        if not self.env.context.get('compute_triggered_by_type_change', False):
            ...
            employees_to_compute = self.filtered(lambda emp: emp.ths_is_medical is None or not emp.id)
        ...
        type_key_to_medical_flag = {rec.employee_type_key: rec.ths_is_medical for rec in config_recs}
        ...
        for employee in employees_to_compute:
            ...
            employee.ths_is_medical = default_is_medical
            _logger.debug(
                f"Computed ths_is_medical for Emp {employee.id} (Type: {employee_type_key}) -> {default_is_medical}")

    def _inverse_ths_is_medical(self):
    ...
    def create(self, vals_list):
        ...
        # ths_is_medical will be computed automatically after creation
        employees = super(HrEmployee, self).create(vals_list)
        for employee in employees:
            if employee.ths_add_to_calendar and employee.ths_is_medical:
    ...
    def write(self, vals):
        ...
        # Store previous state if ths_add_to_calendar or ths_is_medical changes
        prev_state = {}
        if 'ths_add_to_calendar' in vals or 'ths_is_medical' in vals:
            for employee in self:
                prev_state[employee.id] = (employee.ths_add_to_calendar, employee.ths_is_medical)

        res = super(HrEmployee, self).write(vals)

        for employee in self:
            current_add_to_calendar = employee.ths_add_to_calendar
            current_is_medical = employee.ths_is_medical
        ...
        if not app_resource:
            ...
            if not app_resource:  # Still not found, try by resource_id (less specific but a fallback)
                app_resource = AppointmentResource.sudo().search([
                    ...
                    ('ths_resource_category', '=', 'practitioner')  # Ensure it's a practitioner type if found
                ], limit=1)
            ...
            if app_resource.ths_resource_category != 'practitioner': vals_to_update_ar[
                'ths_resource_category'] = 'practitioner'
