...
class ThsPendingPosItem(models.Model):
	...
	- patient_ids = Patients (same as partner_id but M2M, for consistency)
	"""
	...
	def _compute_sub_total(self):
		...
		for item in self:
			if item.discount:
				# discount_amount = (item.price_unit * item.qty) * (item.discount / 100.0)
				item.sub_total = (item.price_unit * item.qty) * (1 - (item.discount / 100.0))
			else:
	...
	def write(self, vals):
		...
		for item in self:
			if item.encounter_id:
				...
				patients_to_add = [
					patient_id for patient_id in updated_patient_ids
					if patient_id not in current_encounter_patient_ids
				]
				...
				if item.room_id and \
						(not encounter.room_id or encounter.room_id.id != item.room_id.id):
