...
class ResourceResource(models.Model):
    ...
    # Link back to the treatment room (set by ths_treatment_room logic)
    ...
    @api.depends('employee_id', 'ths_treatment_room_id')
    def _compute_appointment_type(self):
        ...
        for resource in self:
            ...
            if resource.employee_id and resource.employee_id.ths_is_medical:
                appointment_type = self.env['appointment.type'].search([
                    ('ths_practitioner_ids', 'in', resource.id),
            ...
            if resource.ths_treatment_room_id:
                appointment_type = self.env['appointment.type'].search([
                    ('ths_location_ids', 'in', resource.id),
                    ('schedule_based_on', '=', 'resources')
                ], limit=1)
                resource.appointment_type_id = appointment_type.id if appointment_type else False
                continue
