...
class ProductTemplate(models.Model):
	...
	ths_product_sub_type_id = fields.Many2one(
	...
	def _compute_sub_type_domain(self):
		""" Compute domain string for ths_product_sub_type_id based on the 'type' field (Goods/Service/Combo). """
		for record in self:
	...
	def _onchange_type_clear_sub_type(self):
		...
		if self.ths_product_sub_type_id:
			current_sub_type_is_service = self.ths_product_sub_type_id.is_for_service
			...
			if type_is_service and not current_sub_type_is_service:
				_logger.debug(
					f"Product {self.name or 'New'}: Type changed to Service, clearing non-service Sub Type {self.ths_product_sub_type_id.name}")
				self.ths_product_sub_type_id = False
			# If type is goods (consu) but sub_type *is* for service, clear sub_type
			elif type_is_goods and current_sub_type_is_service:
				_logger.debug(
					f"Product {self.name or 'New'}: Type changed to Goods, clearing service Sub Type {self.ths_product_sub_type_id.name}")
				self.ths_product_sub_type_id = False
...
class ProductProduct(models.Model):
	...
	ths_product_sub_type_id = fields.Many2one(related='product_tmpl_id.ths_product_sub_type_id')
