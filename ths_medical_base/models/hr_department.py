...
class HrDepartment(models.Model):
    ...
    ths_is_medical_dep = fields.Boolean(
    ...
    def create(self, vals_list):
        ...
        for department in departments.filtered(lambda d: d.ths_is_medical_dep):
            department._create_or_update_medical_appointment_type()
        return departments

    def write(self, vals):
        departments_becoming_non_medical = self.env['hr.department']
        if 'ths_is_medical_dep' in vals and not vals['ths_is_medical_dep']:
            departments_becoming_non_medical = self.filtered(lambda d: d.ths_is_medical_dep)

        res = super(HrDepartment, self).write(vals)

        for department in departments_becoming_non_medical:
            if not department.ths_is_medical_dep:
                department._deactivate_medical_appointment_type()

        if ('ths_is_medical_dep' in vals and vals.get('ths_is_medical_dep')) or \
                ('name' in vals and any(
                    self.filtered(lambda d: d.id == record.id).ths_is_medical_dep for record in self)):
            for department in self.filtered(lambda d: d.ths_is_medical_dep):
                department._create_or_update_medical_appointment_type()
        return res
    ...
    def _create_or_update_medical_appointment_type(self):
        self.ensure_one()
        if not self.ths_is_medical_dep:
            return
        ...
        # This assumes ths_treatment_room.appointment_resource_id is populated correctly
        location_appointment_resources = AppointmentResource.search([
            ('ths_treatment_room_id.department_id', '=', self.id),
