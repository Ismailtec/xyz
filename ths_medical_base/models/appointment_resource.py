...
class AppointmentResource(models.Model):
    ...
    ths_resource_category = fields.Selection([
    ...
    ths_treatment_room_id = fields.Many2one(
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            ...
            elif vals.get('employee_id') and vals.get('ths_resource_category') == 'practitioner' and 'name' not in vals:
                employee = self.env['hr.employee'].browse(vals.get('employee_id'))
                if employee.exists():
                    vals['name'] = employee.name

            # If a ths_treatment_room_id is being passed (less direct, room object should be created first)
        ...
        for record in records:
            ...
            if record.ths_resource_category == 'practitioner':
            ...
            elif record.ths_resource_category == 'location':
                ...
                if record.ths_treatment_room_id and record.name != record.ths_treatment_room_id.name:
                    record.sudo().write({'name': record.ths_treatment_room_id.name})
        return records

    # Constraints or other logic can be added as needed
    @api.constrains('ths_resource_category', 'employee_id', 'ths_treatment_room_id', 'resource_id')
    def _check_medical_resource_consistency(self):
        for record in self:
            if record.ths_resource_category == 'practitioner':
                ...
                if record.ths_treatment_room_id:
                ...
                if not record.ths_treatment_room_id:
                ...
                if record.resource_id and record.resource_id.ths_treatment_room_id != record.ths_treatment_room_id:
                    _logger.warning(
                        _("Location Appointment Resource %s links to Room %s, but its underlying resource.resource links to room %s."),
                        record.name, record.ths_treatment_room_id.name, record.resource_id.ths_treatment_room_id.name)